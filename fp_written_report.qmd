...
**Note**: Because of both conditions not met, the test statistics $\chi^2$ may neither have a Chi-square Distribution nor be accurate, meaning that our calculated p-value may not be accurate. If it is smaller than it should be, we may be more likely to reject the null hypothesis when it is true, which is a Type 1 error. If it is bigger than it should be, we may be more likely to fail to reject the null hypothesis when the alternative hypothesis is true, which is a Type 2 error. However, we will still run the Two-Way Table Inference of Independence.
