...
There are 2 expected counts that do not meet the criteria of $e_{i,j} \geq 5$ - $e_{2,1}$ and $e_{3,1}$ - as shown in **Table 2**.
...
Our null distribution (as shown above) is a **Chi-square Distribution** which is always right-skewed, only haves positive values, and has a parameter of 
...
Given that we only take the right tail area to calculate p-value, our $p-value = .006615$.
